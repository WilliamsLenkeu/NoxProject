...
name_user_file = "user"
name_pwd_file = "password"


def recuperation_user():
    import pickle
    import os
    if os.path.exists(name_user_file):
        user_file = open(name_user_file, "rb")
        my_de_pick = pickle.Unpickler(user_file)
        user = my_de_pick.load()
        user_file.close()
    else:
        user = {}
    return user
...
def save_user(user, pwd):
    import pickle
    file_user = open(name_user_file, "wb")
    mon_pickler = pickle.Pickler(file_user)
...
def recuperation_pwd():
    import pickle
    import os
    if os.path.exists(name_pwd_file):
        pwd_file = open(name_pwd_file, "rb")
        my_de_pick = pickle.Unpickler(pwd_file)
        pwd = my_de_pick.load()
        pwd_file.close()
    else:
        pwd = {}
...
def save_pwd(pwd):
    import pickle
    pwd_file = open(name_pwd_file, "wb")
    mon_pickler = pickle.Pickler(pwd_file)
...
def game():
    ...
    *******************************************************************************
    ...
    if choice1 == "gauche":
        choice2 = input(
            'Vous êtes arrivés au lac. Il y a une île au milieu du lac. "attendre" pour attendre un bateau ou "nager" pour y aller à la nage. \n').lower()
        if choice2 == "attendre":
            choice3 = input(
                "Vous êtes arrivés à l'île sans arme. Il y a une maison avec trois portes. Une rouge, Une jaune et une bleu. Quelle couleur choissisez vous? \n").lower()
            if choice3 == "rouge":
